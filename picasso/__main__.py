...
def _join(files: list[str], keep_index: bool = True) -> None:
    ...
    all_locs = pd.concat(all_locs, ignore_index=True)
    all_locs.sort_values(kind="quicksort", by="frame", inplace=True)
...
def _localize_fit_locs(
    ...
    """Dispatch to the appropriate fitting routine and return locs."""
    from time import sleep
    from .localize import get_spots, fit_async, locs_from_fits
    from . import gausslq, avgroi

    if fit_method == "lq" or fit_method == "lq-3d":
        spots = get_spots(movie, ids, box, camera_info)
        theta = gausslq.fit_spots_parallel(spots, asynch=False)
        return gausslq.locs_from_fits(ids, theta, box, gain)
    elif fit_method == "lq-gpu" or fit_method == "lq-gpu-3d":
        spots = get_spots(movie, ids, box, camera_info)
        theta = gausslq.fit_spots_gpufit(spots)
        em = camera_info["Gain"] > 1
        return gausslq.locs_from_fits_gpufit(ids, theta, box, em)
    elif fit_method == "mle":
        ...
        return locs_from_fits(ids, thetas, CRLBs, likelihoods, iterations, box)
    elif fit_method == "avg":
        spots = get_spots(movie, ids, box, camera_info)
        theta = avgroi.fit_spots_parallel(spots, asynch=False)
        return avgroi.locs_from_fits(ids, theta, box, gain)
    else:
        print("This should never happen...")
        return None


def _localize_process_file(  # noqa: C901
    path: str,
    i: int,
    n_total: int,
    args: argparse.Namespace,
