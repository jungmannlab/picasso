...
def _hdbscan(files, min_samples, min_cluster_size):
    import glob
    paths = glob.glob(files)
    if paths:
        from . import io, postprocess
        ...
        for path in paths:
            ...
            locs, info = io.load_locs(path)
            clusters, locs = postprocess.hdbscan(locs, min_samples, min_cluster_size)
            base, ext = os.path.splitext(path)
            hdbscan_info = {'Generated by': 'Picasso HDBSCAN',
...
def main():
    ...
    # HDBSCAN
    hdbscan_parser = subparsers.add_parser('hdbscan', help='cluster localizations with the dbscan clustering algorithm')
    hdbscan_parser.add_argument('files', help='one or multiple hdf5 localization files specified by a unix style path pattern')
    hdbscan_parser.add_argument('min_samples', type=int, help='measure of how conservative the clustering should be')
    hdbscan_parser.add_argument('min_cluster_size', type=int, help='minimum local density for localizations to be assigned to a cluster')
    ...
    if args.command:
        if args.command == 'toraw':
            ...
            _hdbscan(args.files, args.min_samples, args.min_cluster_size)
