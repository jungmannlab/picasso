...
def _undrift(files, mode, segmentation, display):
    import glob
    paths = glob.glob(files)
    if paths:
        from picasso import io, postprocess
        ...
        for path in paths:
            ...
            drift, corrected_locs = postprocess.undrift(locs, info, segmentation, mode=mode, movie=movie, display=display)
            base, ext = os.path.splitext(path)
...
if __name__ == '__main__':
    ...
    if args.command:
        if args.command == 'toraw':
            ...
            _undrift(args.files, args.mode, args.segmentation, args.nodisplay)
