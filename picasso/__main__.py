...
def _split(files, frame, max):
    ...
    paths = glob.glob(files)
    if paths:
        from picasso.io import load_raw, save_info
        from os.path import splitext
        ...
        for path in paths:
            ...
            base, ext = splitext(path)
            for i, (subregion, shift, info, wavelength) in enumerate(zip(*splits)):
                subregion_base = base + '_split{}'.format(wavelength)
                subregion.tofile(subregion_base + '.raw')
                info[0]['Excitation Wavelength'] = wavelength
                subregion_info = {'Generated by': 'Picasso Split',
                                  'X0': shift[1],
                                  'Y0': shift[0]}
                save_info(subregion_base + '.yaml', info[:] + [subregion_info])
