...
def _localize(files, parameters_file, verbose=True):
    ...
    if n_files:
        ...
        for i, path in enumerate(paths):
            ...
            info = [info, parameters.update({'Generated by': 'Picasso Localize CLI'})]
...
def _link(files, radius, max_dark_time):
    paths = glob.glob(files)
    for path in paths:
        locs, info = io.load_locs(path)
        linked_locs = postprocess.link(locs, radius, max_dark_time)
        base, ext = os.path.splitext(path)
        link_info = {'Radius': radius,
                     'Maximum Transient Dark Time': max_dark_time,
                     'Generated by': 'Picasso Link CLI'}
...
if __name__ == '__main__':
    ...
    link_parser.add_argument('-d', '--darktime', type=int,
    ...
    if args.command:
        if args.command == 'toraw':
            ...
            _link(args.files, args.radius, args.darktime)
    else:
        parser.print_help()
