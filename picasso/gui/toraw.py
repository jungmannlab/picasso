...
class Window(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        files_label.setToolTip("List of files to convert.")
        vbox.addWidget(files_label)
        self.path_edit = TextEdit()
        vbox.addWidget(self.path_edit)
        hbox = QtWidgets.QHBoxLayout()
        vbox.addLayout(hbox)
        self.browse_button = QtWidgets.QPushButton("Browse")
        self.browse_button.setToolTip("Browse for files to convert.")
        self.browse_button.clicked.connect(self.browse)
        hbox.addWidget(self.browse_button)
        hbox.addStretch(1)
        to_raw_button = QtWidgets.QPushButton("To raw")
        to_raw_button.setToolTip("Convert selected files to raw format.")
