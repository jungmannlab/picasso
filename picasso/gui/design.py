...
import csv
BaseSequencesFile = 'BaseSequences.csv'
...
def plotPlate(selection,platename):
    ...
    ax.set_yticks([])
    inch = 25.4
    xsize = 13*2*radius/inch*1.4 #Dont ask why
    ysize = 9*2*radius/inch*1.4
...
class Scene(QtGui.QGraphicsScene):
    ...
    def __init__(self, window):
        ...
        for coords in BINDING_SITES:
            ...
            x = coords[1]
            hex_center_x, hex_center_y = indextoHex(y,x)
            self.allcords.append((hex_center_x*0.125,hex_center_y*0.125)) #5nm ORIGAMI GRID, half is 20, should be 2.5
