...
class MaskSettingsDialog(QtWidgets.QDialog):
    ...
    def _mask_locs(self, locs: pd.DataFrame) -> None:
        ...
        ) or not self.save_all.isChecked():  # update masked locs plot if the current channel is masked
...
class Window(QtWidgets.QMainWindow):
    ...
    def initUI(self, plugins_loaded: bool) -> None:
        ...
        group_action = postprocess_menu.addAction("Remove columns")
        group_action.triggered.connect(self.remove_columns)
    ...
    def remove_columns(self) -> None:
        ...
        if channel is not None:
            ...
            info = self.view.infos[channel]
            columns, ok = RemoveColumnsDialog.getParams(
                self, locs.columns.to_list()
            )
            print(columns, ok)
            if not ok:
                return
