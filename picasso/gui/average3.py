...
print(
    "Deprecation Warning: The module 'average3' is deprecated and will be removed in version 1.0. Please consider using alternative tools for 3D averaging."
)
...
class Window(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.rotatexy_convbtn = QtWidgets.QPushButton(
            "Rotate XY - Convolution"
        )
    ...
    def save(self, path):
        ...
        for i in range(n_channels):
            ...
            out_locs.y += cy
            info = self.infos[i] + [
                {"Generated by": f"Picasso v{__version__} Average3"}
            ]

            if not self.z_state[i]:
                out_locs.drop(columns="z", inplace=True)
    ...
    def centerofmass_all(self):
        ...
        out_locs_x = pd.concat(out_locs_x, ignore_index=True)
        out_locs_y = pd.concat(out_locs_y, ignore_index=True)
        out_locs_z = pd.concat(out_locs_z, ignore_index=True)
            ...
            if self.dataset_dialog.checks[j].isChecked():
                ...
                for k in range(n_angles):
                    ...
                    if alignimage == []:
                        ...
                        if alignaxis == "zz":
                            alignimage[
                                np.int64(alignimage.shape[0] / 2), :
                            ] += 2
                            alignimage[
                                np.int64(alignimage.shape[0] / 2) + 1, :
                            ] += 1
                            alignimage[
                                np.int64(alignimage.shape[0] / 2) - 1, :
                            ] += 1
                            ...
                            alignimage[
                                :, np.int64(alignimage.shape[1] / 2)
                            ] += 2
                            alignimage[
                                :, np.int64(alignimage.shape[1] / 2) - 1
                            ] += 1
                            alignimage[
                                :, np.int64(alignimage.shape[1] / 2) + 1
                            ] += 1
