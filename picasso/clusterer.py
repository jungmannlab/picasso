...
def find_cluster_centers(
    ...
    n_events = np.array([_[15] for _ in centers_]) # number of localizations in cluster
...
def cluster_center(
    ...
    split_idx = np.where(np.diff(grouplocs.frame) > 3)[0] + 1 # split locs by consecutive frames
