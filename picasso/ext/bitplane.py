...
if IMSWRITER:
    ...
    class MovieMapper():
        ...
        def __getitem__(self, item):
            return self.file['DataSet'][self.RL][self.lookup_dict[item]][self.channel]['Data'][0][:self.y, :self.x]
        ...
        def __iter__(self):
            for item in range(len(self.frames)):
                yield self.file['DataSet'][self.RL][self.lookup_dict[item]][self.channel]['Data'][0][:self.y, :self.x]
    ...
    class MovieMapperStack():
        ...
        def __getitem__(self, item):
            return self.file['DataSet'][self.RL][self.frames[0]][self.channel]['Data'][item, :, :]
        ...
        def __iter__(self):
            for item in range(self.n_frames):
                yield self.file['DataSet'][self.RL][self.frames[0]][self.channel]['Data'][item, :, :]
    ...
    class IMSFile:
        ...
        def set_channel(self, channel):
            self.channel = channel
            self.img_size = np.array(self.file['DataSet'][self.RL][self.frames[0]][self.channel]['Data']).shape
            self.dtype = np.array(self.file['DataSet'][self.RL][self.frames[0]][self.channel]['Data']).dtype

            try:
                z = int(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['Z']]))
            except KeyError:
                z = self.img_size[0]
            self.z = z

            try:
                self.x = int(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['X']]))
                self.y = int(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['Y']]))
            except KeyError:
            ...
            ext_max0 = float(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['ExtMax0']]))
            ext_min0 = float(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['ExtMin0']]))

            ext_max1 = float(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['ExtMax1']]))
            ext_min1 = float(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['ExtMin1']]))

            ext_max2 = float(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['ExtMax2']]))
            ext_min2 = float(''.join([_.decode() for _ in self.file['DataSetInfo']['Image'].attrs['ExtMin2']]))
            ...
            print(f'Image dimensions X: {delta_x} um Y: {delta_y} um. Pixelsize x {px_x}, y {px_y}, p {px_nm}')
        ...
        def read_frames(self):
            if not self.frames_read:
                self.frames_int = np.array([int(_.split('TimePoint ')[1]) for _ in self.frames])
        ...
        def read_frame(self, frame):
            if not self.lookup_dict:
                self.read_frames()
            return self.file['DataSet'][self.RL][self.lookup_dict[frame]][self.channel]['Data'][0]

        def read_z_stack(self):
            print('Reading stack')
            self.n_frames = self.z
            self.movie = MovieMapperStack(self.file, self.RL, self.channel, self.frames, self.dtype, self.n_frames)
        ...
        def read_stack(self):
            ...
            self.movie = MovieMapper(self.file, self.RL, self.lookup_dict, self.channel, self.frames, self.x, self.y, self.dtype)
    ...
    def numpy_to_imaris(array, filename, colors, oversampling, viewport, info, z_min, z_max, pixelsize):
        ...
        converter = PW.ImageConverter(dtype_imaris, image_size, sample_size, dimension_sequence, block_size,
