...
if os.name == "nt":
    lib_path = os.path.join(
        package_dir, "Gpufit.dll"
    )  # library name on Windows
elif os.name == "posix":
    lib_path = os.path.join(
        package_dir, "libGpufit.so"
    )  # library name on Unix
else:
...
def _valid_id(cls, id):
    properties = [
        key for key in cls.__dict__.keys() if not key.startswith("__")
    ]
...
def fit_constrained(
    ...
    if not initial_parameters.flags.c_contiguous:
        raise RuntimeError(
            "Memory layout of initial_parameters array mismatch."
        )
    ...
    if (
        constraint_types is not None
        and constraint_types.shape[0] != number_parameters
    ):
        raise RuntimeError(
    ...
    if not _valid_id(EstimatorID, estimator_id):
        raise RuntimeError(
            "Invalid estimator ID, use an attribute of EstimatorID"
        )
    if not all(
    ...
    if status != Status.Ok:
        ...
        raise RuntimeError(
            "status = {}, message = {}".format(status, error_message)
        )
...
def get_cuda_version():
    ...
    status = get_cuda_version_func(
        byref(runtime_version), byref(driver_version)
    )
