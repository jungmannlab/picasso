...
def comet(
    ...
    *,
    ...
    if missing:
        raise KeyError(f"Missing required columns for COMET: {sorted(missing)}")
    ...
    dataset[:, 2] = (
        locs["z"].to_numpy(dtype=np.float64) if has_z else 0.0
    )
    dataset[:, 3] = frame0
    ...
    locs["x"] = locs["x"].to_numpy(dtype=np.float64) - drift_nm[frame_idx, 0] / pixelsize_nm
    locs["y"] = locs["y"].to_numpy(dtype=np.float64) - drift_nm[frame_idx, 1] / pixelsize_nm
    if has_z:
        locs["z"] = locs["z"].to_numpy(dtype=np.float64) - drift_nm[frame_idx, 2]
...
if _CUDA_AVAILABLE:
    @_cuda.jit
    def cost_function_full_3d_chunked(d_locs_time, start_idx, chunk_size, d_idx_i, d_idx_j, d_sigma,
        ...
        if pos < chunk_size:
            ...
            _cuda.atomic.add(d_deri, (tj, 0), 2 * val * (d_locs_coords[j, 0] - d_locs_coords[i, 0] + mu[ti, 0] - mu[tj, 0]) / sigma_sq)
            _cuda.atomic.add(d_deri, (tj, 1), 2 * val * (d_locs_coords[j, 1] - d_locs_coords[i, 1] + mu[ti, 1] - mu[tj, 1]) / sigma_sq)
            _cuda.atomic.add(d_deri, (tj, 2), 2 * val * (d_locs_coords[j, 2] - d_locs_coords[i, 2] + mu[ti, 2] - mu[tj, 2]) / sigma_sq)

            _cuda.atomic.add(d_deri, (ti, 0), 2 * val * (d_locs_coords[i, 0] - d_locs_coords[j, 0] + mu[tj, 0] - mu[ti, 0]) / sigma_sq)
            _cuda.atomic.add(d_deri, (ti, 1), 2 * val * (d_locs_coords[i, 1] - d_locs_coords[j, 1] + mu[tj, 1] - mu[ti, 1]) / sigma_sq)
            _cuda.atomic.add(d_deri, (ti, 2), 2 * val * (d_locs_coords[i, 2] - d_locs_coords[j, 2] + mu[tj, 2] - mu[ti, 2]) / sigma_sq)

            _cuda.atomic.add(d_val_sum, 0, val)
            d_val[pos] = 0

    def cuda_wrapper_chunked(mu, d_locs_coords, d_locs_time, d_idx_i, d_idx_j, d_sigma, d_sigma_factor, d_val,
                             d_deri, chunk_size):
        ...
        mu_dev = _cuda.to_device(np.asarray(mu.reshape(int(mu.size / 3), 3), dtype=np.float64))
