...
The original publication by `Smith et al., 2010 <https://doi.org/10.1038/nmeth.1449>`_ describes a maximum likelihood estimation (MLE) fitting procedure for localizing single molecules. Picasso adapts this approach, however, it fits the Gaussian σ as a free parameter. Picasso Localize supports anisotropic σ fitting (i.e., :math:`\sigma_x \neq \sigma_y`), however, Picasso API provides support for the isotropic σ fitting (i.e., :math:`\sigma_x = \sigma_y`).
