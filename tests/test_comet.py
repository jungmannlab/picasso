...
def minimal_locs_structured(minimal_locs_df):
    ...
    df = minimal_locs_df
    dtype = np.dtype([("frame", np.uint32), ("x", np.float32), ("y", np.float32)])
...
class TestSegmentationByLocs:
    def test_produces_correct_segment_count(self):
        ...
        result = comet.segmentation_wrapper(
            frames, segmentation_var=50, segmentation_mode=1, return_param_dict=True
        )
    ...
    def test_max_locs_per_segment_respected(self):
        ...
        cap = 30
        result = comet.segmentation_wrapper(
            frames, segmentation_var=200, segmentation_mode=1,
            max_locs_per_segment=cap, return_param_dict=True
        )
...
class TestSegmentationByNumWindows:
    def test_num_windows_mode_creates_requested_count(self):
        ...
        result = comet.segmentation_wrapper(
            frames, segmentation_var=5, segmentation_mode=0, return_param_dict=True
        )
...
class TestPairIndicesKdtree:
    def test_finds_close_pairs(self):
        coords = np.array([[0, 0, 0], [1, 0, 0], [100, 0, 0]], dtype=np.float32)
...
class TestInterpolateDrift:
    ...
    def test_catmull_rom_output_shape(self):
        ...
        out = comet.interpolate_drift(cf, de, frame_range, method="catmull-rom")
    ...
    def test_catmull_rom_requires_min_points(self):
        ...
        with pytest.raises(ValueError, match="at least 4 points"):
            comet.interpolate_drift(cf, de, np.arange(20), method="catmull-rom")
...
class TestCometNoCuda:
    ...
    def test_raises_runtime_error_without_cuda(self, minimal_locs_df, minimal_info):
        if comet._CUDA_AVAILABLE:
            pytest.skip("CUDA is present; testing no-GPU path only on CPU machines")
        with pytest.raises(RuntimeError, match="CUDA"):
            comet.comet(minimal_locs_df, minimal_info, locs_per_segment=100, max_drift_nm=200)

    def test_error_message_mentions_alternatives(self, minimal_locs_df, minimal_info):
        if comet._CUDA_AVAILABLE:
            pytest.skip("CUDA is present; testing no-GPU path only on CPU machines")
        with pytest.raises(RuntimeError, match="AIM|RCC"):
            comet.comet(minimal_locs_df, minimal_info, locs_per_segment=100, max_drift_nm=200)

    def test_structured_array_input_accepted_before_gpu_check(self, minimal_locs_structured, minimal_info):
        ...
        if comet._CUDA_AVAILABLE:
            pytest.skip("CUDA is present; testing no-GPU path only on CPU machines")
        with pytest.raises(RuntimeError, match="CUDA"):
            comet.comet(minimal_locs_structured, minimal_info, locs_per_segment=100, max_drift_nm=200)
    ...
    def test_missing_columns_raises_key_error(self, minimal_info):
        ...
        if comet._CUDA_AVAILABLE:
            pytest.skip("CUDA is present; GPU path reached before column check")
        bad_locs = pd.DataFrame({"frame": np.arange(10), "x": np.zeros(10)})
        # No 'y' column — should raise KeyError from the column check
        with pytest.raises((KeyError, RuntimeError)):
            comet.comet(bad_locs, minimal_info, locs_per_segment=5, max_drift_nm=100)
    ...
    def test_z_column_accepted(self, minimal_locs_df, minimal_info):
        ...
        with pytest.raises(RuntimeError, match="CUDA"):
            comet.comet(df, minimal_info, locs_per_segment=100, max_drift_nm=200)
    ...
    def test_missing_pixelsize_raises_on_gpu(self, minimal_locs_df):
        ...
        with pytest.raises((KeyError, RuntimeError, ValueError)):
            comet.comet(minimal_locs_df, bad_info, locs_per_segment=100, max_drift_nm=200)
...
class TestCometFullPipeline:
    ...
    def test_output_shapes(self, minimal_locs_df, minimal_info):
        undrifted, new_info, drift = comet.comet(
            minimal_locs_df, minimal_info,
            locs_per_segment=100, max_drift_nm=300,
            target_sigma_nm=50,
        )
    ...
    def test_structured_array_gives_same_result_as_dataframe(self, minimal_locs_df, minimal_locs_structured, minimal_info):
        locs_df, _, drift_df = comet.comet(
            minimal_locs_df, minimal_info,
            locs_per_segment=100, max_drift_nm=300, target_sigma_nm=50,
        )
        locs_arr, _, drift_arr = comet.comet(
            minimal_locs_structured, minimal_info,
            locs_per_segment=100, max_drift_nm=300, target_sigma_nm=50,
        )
        np.testing.assert_allclose(drift_df["x"].to_numpy(), drift_arr["x"].to_numpy(), rtol=1e-5)
